...
public class Application {
    ...
    ///Debug functions
    public static Date outlierMapContainsEdge(Map<Date, Set<Segment>> outliers, int edgeId) {
        for(Map.Entry<Date, Set<Segment>> current: outliers.entrySet()) {
            for (Segment s: current.getValue()) {
                if (s.id == edgeId) {
    ...
    public static Date regionsContainEdge(Map<Date, List<Subgraph>> regions, int edgeId) {
        for(Map.Entry<Date, List<Subgraph>> current: regions.entrySet()) {
            for (Subgraph sg: current.getValue()) {
                for (LabeledDefaultEdge le: sg.getEdges()) {
                    if (le.label==edgeId) {
                        return current.getKey();
                    }
                }
    ...
    public static Subgraph listContainsEdge(List<Subgraph> graphs, int edgeId) {
        for (Subgraph sg: graphs) {
            for (LabeledDefaultEdge edge: sg.getEdges()) {
                if (edge.label == edgeId) {
                    return sg;
                }
            }
        }
    ...
    public static Segment setContainsSegment(Set<Segment> seg, int id) {
        for (Segment s: seg) {
            if (s.id == id) {
                return s;
            }
    ...
    public static LabeledDefaultEdge setContainsEdge(Set<LabeledDefaultEdge> set, int id) {
        for (LabeledDefaultEdge edge: set) {
            if (edge.label==id) {
                return edge;
            }
        }
